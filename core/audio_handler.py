...
class AudioHandler:
    ...
    def save_uploaded_file(self, file, timestamp=None):
        ...
        try:
            ...
            self.queue_for_processing(session_id, filename, str(filepath), file_size, timestamp)
    ...
    def queue_for_processing(self, session_id, filename, filepath, file_size, timestamp):
        ...
        try:
            ...
            self.redis_client.set_session_status(session_id, {
                'status': 'queued',
                'stream_id': stream_id,
    ...
    def get_processed_file_path(self, session_id):
        """Get the path to the processed WAV file"""
        try:
            ...
            logger.error(f"Error getting processed file path: {e}")
            return None
    
    def cleanup_session_files(self, session_id):
        """Clean up uploaded and processed files for a session"""
        try:
            status_data = self.get_session_status(session_id)
            if not status_data:
                return False
            ...
            if 'output_path' in status_data:
                files_to_clean.append(status_data['output_path'])
    ...
    def get_system_stats(self):
        ...
        try:
            ...
            stats = {
                ...
                'pending_messages': len(self.redis_client.get_pending_messages(stream_name, consumer_group)),
                'upload_folder_size': self._get_folder_size(current_app.config['UPLOAD_FOLDER']),
                'processed_folder_size': self._get_folder_size(current_app.config['PROCESSED_FOLDER'])
            }
